...
export const createReimbrushment = async (req, res) => {
  try {
    ...
    if (
      req.files["reimbursementDoc"] &&
      req.files["reimbursementDoc"].length > 0
    ) {
...
export const reimbrushmentList = async (req, res) => {
  try {
    ...
    reimbrushment = await reimbrushmentModel
      .find(query)
    ...
    reimbrushment = reimbrushment.map((item) => {
      ...
      let reimbrushmentDocument =
        process.env.IMAGE_BASE_URL + item.reimbrushmentDocument;
  ...
  try {
    ...
    reimbrushment = reimbrushment.map((item) => {
      ...
      return {
        ...
        amount: `INR ${item.amount}`, // Format the amount with a dollar sign
        // reimbDate: item.reimbDate
        //   ? new Date(item.reimbDate).toLocaleDateString("en-GB", {
        //       day: "2-digit",
        //       month: "short",
        //       year: "numeric",
        //     })
    ...
    const result = await reimbrushmentModel.aggregate([
      ...
      },
      {
        $group: {
          ...
          all: { $sum: "$amount" },
          pending: {
            $sum: { $cond: [{ $eq: ["$status", "0"] }, "$amount", 0] },
          },
          approved: {
            $sum: { $cond: [{ $eq: ["$status", "1"] }, "$amount", 0] },
          },
          rejected: {
            $sum: { $cond: [{ $eq: ["$status", "2"] }, "$amount", 0] },
          },
        },
      },
    ]);
...
export const reimbrushmentUpdate = async (req, res) => {
  try {
    ...
    const { reimbId, vendorId, reimbDate, reimbType, notes, amount, type } =
      req.body;
    ...
    res
      .status(500)
      .json({ message: "Reimbursement update failed", error: error.message });
  }
};
  ...
  try {
    ...
    if (status != 1 && status != 2) {
      return res.status(400).json({
        error:
          "Invalid status. Please pass 1 (Approved), or 2 (Rejected) only.",
      });
    }
    // Retrieve the existing reimbursement record
    const existingReimb = await reimbrushmentModel.findById(reimbId);
    ...
    res
      .status(200)
      .json({ message: `Reimbursement ${statusText} successfully` });
  } catch (error) {
    console.error(error);
    res
      .status(500)
